            ...
            <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 10px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                <tr>
                    <td align="center" style="padding-bottom: 20px;">
                        <div style="width: 60px; height: 60px; background-color: {{$instrumentIndex->status_updated == 'approve' ? 'green' : 'red'}}; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto;">
                            <span style="font-size: 32px; color: white;">{{$instrumentIndex->status_updated == 'approve' ? '✓' : 'X'}}</span>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td style="font-size: 18px; color: #333333; padding-bottom: 20px;">
                        Dear {{$requestor->engineers?->name}},
                    </td>
                </tr>
                <tr>
                    <td style="font-size: 16px; color: #444444; padding-bottom: 20px;">
                        @if($instrumentIndex->status_updated == 'reject')
                            Your Instrument Index update has been rejected due to some issues.
                            Please check the remarks and make the necessary corrections by clicking <a href=""> here </a>
                        @else
                            Your loop number request has been <strong>approved</strong>
                        @endif
                    </td>
                </tr>
                @if($instrumentIndex->status_updated == 'reject')
                    <tr>
                        <td style="font-size: 16px; padding-bottom: 30px;">
                            {{$instrumentIndex->remark_updated}}
                        </td>
                    </tr>
                @endif
                <tr>
